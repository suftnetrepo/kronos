...
import {
  ...
  StyledDivider, StyledEmptyState, StyledSkeleton, Switch,
  StyledPage,
  theme,
} from 'fluent-styles'
...
function dueDateLabel(date: Date): { text: string; color: string } {
  if (isToday(date))     return { text: 'Due today',    color: '#F97316' }
  if (isTomorrow(date))  return { text: 'Due tomorrow', color: '#EAB308' }
  if (isPast(date))      return { text: 'Overdue',      color: '#EF4444' }
  return { text: format(date, 'MMM d'), color: '#6B7280' }
}

function HomeworkRow({
  hw, subjectName, subjectColor, onToggle, onDelete,
}: {
  hw:           Homework
  subjectName:  string
  subjectColor: string
  ...
  const due     = dueDateLabel(new Date(hw.dueDate))
  ...
  return (
    <StyledPressable onLongPress={() => onDelete(hw)}>
      <Stack flexDirection="row" alignItems="center" gap={14}
        ...
        >
          {hw.done && <StyledText fontSize={13} color="#fff">✓</StyledText>}
        </StyledPressable>
        ...
        <Stack flex={1} gap={4}>
          ...
          <Stack flexDirection="row" alignItems="center" gap={8}>
            <Stack
              ...
              borderRadius={6} backgroundColor={subjectColor + '20'}>
              <StyledText fontSize={11} fontWeight="700" color={subjectColor}>
                {subjectName}
              </StyledText>
            </Stack>
            <StyledText fontSize={11} fontWeight="600" color={due.color}>
              {due.text}
            </StyledText>
          </Stack>
        </Stack>
...
export default function HomeworkScreen() {
  const Colors = useColors()
  const { data: homeworks, loading, refetch, toggleDone, remove } = useHomework()
  ...
  const handleDelete = useCallback(async (hw: Homework) => {
    const ok = await dialogueService.confirm({
      title: 'Delete homework?',
      message: `"${hw.title}" will be removed.`,
      icon: '🗑️', confirmLabel: 'Delete', destructive: true,
    })
    if (ok) { await remove(hw.id); toastService.success('Deleted') }
  }, [remove])
  ...
  return (
    <StyledPage flex={1} backgroundColor={Colors.bg}>
      <StyledPage.Header
        ...
        rightIcon={
         ( <Stack flexDirection="row" alignItems="center" gap={6}>
          <StyledText fontSize={13} color={Colors.textMuted}>Show done</StyledText>
          <Switch value={showDone} onChange={setShowDone}
            size="sm" activeColor={Colors.primary} />
        </Stack>)
        }
      />
      {loading ? (
        <Stack padding={16}><StyledSkeleton template="list-item" repeat={4} /></Stack>
      ) : homeworks.length === 0 ? (
        <StyledEmptyState variant="minimal" illustration="🎉"
          title="No homework!" description="Enjoy your free time 🎊" animated />
      ) : (
        <ScrollView contentContainerStyle={{ paddingTop: 16, paddingHorizontal: 16, paddingBottom: 140 }}>
          {/* Pending */}
          {pending.length > 0 && (
            <StyledCard
              marginBottom={8}
              ...
              {pending.map((hw, i) => {
                ...
                return (
                  <Stack key={hw.id}>
                    ...
                    />
                    {i < pending.length - 1 && (
                      <StyledDivider borderBottomColor={Colors.border} marginLeft={56} />
                    )}
                  </Stack>
                )
              })}
      ...
      >
      <StyledText fontSize={28} color={Colors.white} style={{ lineHeight: 32 }}>+</StyledText>
      </StyledPressable>
